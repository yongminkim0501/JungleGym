...
# 비밀 번호 업데이트
@auth_bp.route("/re_password")
@inject
def re_password(
    user_service: UserService = Provide(ApplicationContainers.user_service)
):
    data = RePasswordRequest.models_validate((request.get_json()))
    email = data.email
    password = data.password
    hashed_password = hash_password(password=password)

    return user_service.update_pw_by_email(email=email, password=hashed_password)
